(function () {
    var controllerId = 'app.views.writetheplan';
    angular.module('app').filter('to_trusted', ['$sce', function ($sce) {
        return function (text) {
            return $sce.trustAsHtml(text);
        };
    }]).controller(controllerId, [
        '$scope', '$timeout', 'abp.services.app.planTemplate', 'abp.services.app.plan', function ($scope, $timeout, planTemplateService, planService) {
            var vm = this;
            ...
            //加载预案内容
            vm.LoadPlan = function (id) {
                planService.getSection(id).then(function (result) {
                    vm.PlanName = result.data.planName;
                    vm.oldPlanName = result.data.planName;
                    vm.TypeID = result.data.planTypeID;
            ...
            vm.loadTypes = function () {
                $.getJSON("/Plan/PlanType.json", function (types) {
                    vm.Types = types;
                    vm.TypeID = types[0].id;
                });
            };
            
            //加载模板区域
            vm.loadAreas = function () {
                planService.getUserAreas().then(function (result) {
            ...
            //保存预案
            vm.save = function () {
                if (vm.PlanName == null || vm.PlanName == "") {
                    abp.notify.error("请输入预案名称");
                    return;
                }
                vm.IsSaving = true;
                var postdata = {
                    PlanName: vm.PlanName,
                    PlanTypeID: vm.TypeID,
                    id: vm.id,
                    Sections: []
                };
                ...
                if (parseInt(vm.id) > 0) {
                    planService.save(postdata).then(function (result) {
                        if (result.data == "") {
                    ...
                    planService.addPlan(postdata).then(function (result) {
